...
!real(8), allocatable :: evalfv(:,:,:)     ! L.Z.: re-named to evalfvloc, declared in addons/mod_addons.f90
...
if (tsveqn) then
  allocate(evalfvloc(nstfv,nspnfv,nkptloc))           ! L.Z.: renamed 
...
! solve the secular equation
    if (tsveqn) then
      call seceqn(ikloc,evalfvloc(1,1,ikloc),evecfvloc(1,1,1,ikloc),evecsvloc(1,1,ikloc))    ! L.Z.: renamed
    else
  ...
  do i=0,mpi_grid_dim_size(dim_k)-1
    if (mpi_grid_dim_pos(dim_k).eq.i) then
      do ikloc=1,nkptloc
        ...
        if (tsveqn) then
          call putevalfv(ik,evalfvloc(1,1,ikloc))       ! L.Z.: renamed
...
deallocate(v,work)
if (tsveqn) then
  deallocate(evalfvloc)   ! L.Z.: renamed
