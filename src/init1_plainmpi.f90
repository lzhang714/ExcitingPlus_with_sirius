...
!! ------------------------------------------------------------------------------- !!
!! This subroutine is created ONLY for gndstate_901, which calls sirius-full-scf.  !!
!! We want no mpi distribution of k-points. Each rank has full "nkpt" k-points.    !!
!! ------------------------------------------------------------------------------- !!
...
!!  
!!  By commenting out the above 6 lines and setting nkptloc=nkpt,nkptnrloc=nkptnr, we no longer have k-points distribution. 
!!  In the rest of init1_plainmpi and in the gndstate_901, we also comment out the corresponding mpi-gather operations.
!!   
  nkptloc = nkpt 
  nkptnrloc = nkptnr
  ...
  if (use_sirius_library.and.use_sirius_init) then
    ...
    !! offset in the regular FFT grid buffer (all interstitital arrays). 
...
!---------------------!
  ...
  allocate(ngk(nspnfv,nkpt))                        !! originally "nkpt", not changed 
  if (allocated(igkig)) deallocate(igkig)
  allocate(igkig(ngkmax,nspnfv,nkptloc))            !! changed "nkptloc" to "nkpt"
  if (allocated(vgkl)) deallocate(vgkl)
  allocate(vgkl(3,ngkmax,nspnfv,nkptloc))           !! changed "nkptloc" to "nkpt"
  if (allocated(vgkc)) deallocate(vgkc)
  allocate(vgkc(3,ngkmax,nspnfv,nkptloc))           !! changed "nkptloc" to "nkpt"
  if (allocated(gkc)) deallocate(gkc)
  allocate(gkc(ngkmax,nspnfv,nkptloc))              !! changed "nkptloc" to "nkpt"
  if (allocated(tpgkc)) deallocate(tpgkc)
  allocate(tpgkc(2,ngkmax,nspnfv,nkptloc))          !! changed "nkptloc" to "nkpt"
  if (allocated(sfacgk)) deallocate(sfacgk)
  allocate(sfacgk(ngkmax,natmtot,nspnfv,nkptloc))   !! changed "nkptloc" to "nkpt"
    ...
    do ispn=1,nspnfv
      ...
      if (use_sirius_library.and.use_sirius_gkvec) then
        ...
        call sirius_get_gkvec_arrays(ks_handler, ik, ngk(1, ik), igkig(1, 1, ik),&
                                     ...
                                     &tpgkc(1, 1, 1, ik))
        !EXCITING interface has gensfacgp here when NOT using sirius eigen solver
