...
!BOC
      ...
      do ilo=1,nlorb(is)
        ...
        do io=1,apword(l,is)
          ...
          oalo(io,ilo,ias)=gr(nr)
                    ! Exciting has these lines for valence relativity:
  ...
  if (use_sirius_library.and..not.use_sirius_olprad) then
         ...
         do is = 1, nspecies
           do ia = 1, natoms(is)
             ias = idxas(ia, is)
               ...
               ! Exciting has these lines while I don't have for the moment:
               !if (input%groundstate%ValenceRelativity.eq.'iora*') then
