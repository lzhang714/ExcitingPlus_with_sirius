...
!------------------------!
  ...
  if (use_sirius_library.and.use_sirius_init) then
        ...
        do is = 1, nspecies
          ...
          ! pass radial grid for MT and for effective infinity (check genrmesh, called above before sirius init).
          ! NOTE: radial grid can be set from the EP side or generated by SIRIUS, in the second case 
          !       it must be brought back to the Fortran side and all the relevant arrays must be reallocated. 
          !       So we choose to generated in EP side and pass to SIRIUS.  
          call sirius_set_atom_type_radial_grid(sctx, string(trim(label)), nrmt(is), spr(1, is))
          call sirius_set_atom_type_radial_grid_inf(sctx, string(trim(label)), spnr(is), spr(1, is))
          ...
          !
          ! 1. The 3rd argument can be "-1" which means the library will figure out principle quantum number. 
          ...
          !
          ! 2. remind myself some definitions: 
          ...
          !
          ! 1. The 4th input argument can be "-1" which means the library will try to figure out principal quantum number itself.
          !    It seems we can also use: lopqn(ilo,is), which is also in species file. 
          !    EXCITING has the similar variable: lorbpqn(io, ilo, is), in mod_APW_LO.F90, but I am not sure where its default value comes?
          !
