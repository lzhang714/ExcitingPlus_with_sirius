...
subroutine gndstate_1001
  ...
  !
  !    These newely defined arrays in modmain/init1 are used here in gndstate_1001 !!! 
  !    
  ! ------------------------------------------------------------------------------------------------- 

  ! local variables
  logical exist
  integer ik,is,ia,ias,ist,i,ikloc,idm,inmat
  integer n,nwork
  ...
  evalfv(:,:,:)=0.d0               ! all nkpt 
  evalfv_sirius(:,:,:)=0.d0        ! all nkpt 
  evalsv(:,:)=0.d0                 ! all nkpt 
  evalsv_sirius(:,:)=0.d0          ! all nkpt
  evecfv3(:,:,:,:)=zzero           ! all nkpt, "3" because "evecfv" and "evecfv2" are already used.
  evecfv_sirius(:,:,:,:)=zzero     ! all nkpt
  ...
  if ((task.eq.1).or.(sic.and.isclsic.gt.1)) then
      ...
      if (use_sirius_library.and.sirius_run_full_scf) then
        ...
        enddo
        ! ------------------------------------------------------! 
        ! run full scf from SIRIUS right after initialization   ! 
        ! ------------------------------------------------------! 
    ...
    do i=0,mpi_grid_dim_size(dim_k)-1
      if (mpi_grid_dim_pos(dim_k).eq.i) then
        do ikloc=1,nkptloc
          ...
          evalfv_tmp(:,:)=0.d0   
          evecfv_tmp(:,:,:)=zzero
          ! get global ik from local ikloc
          ik=mpi_grid_map(nkpt,dim_k,loc=ikloc)
          ! retrieve fv eigen values and vectors from sirius, by global index ik. 
          ! SIRIUS API function does not include the dimension for "nspnfv".
          call sirius_get_fv_eigen_values(  ks_handler, ik, evalfv_tmp(1,ik), nstfv )
          call sirius_get_fv_eigen_vectors( ks_handler, ik, evecfv_tmp(1,1,ik), nmatmax, nstfv )
          ! retrieve sv eigen values and vectors from sirius, by global index ik
          if (ndmag.eq.0.or.ndmag.eq.3) then
            call sirius_get_band_energies(ks_handler, ik, 0, evalsv_sirius(1,       ik) )
          else
            call sirius_get_band_energies(ks_handler, ik, 0, evalsv_sirius(1,       ik) )
            call sirius_get_band_energies(ks_handler, ik, 1, evalsv_sirius(nstfv+1, ik) )
          endif
          ! TODO: need SIRIUS API function to retrieve sv eigen vectors
  ...
  call mpi_grid_reduce( evalsv_sirius(1,1),  nstsv*nkpt,          dims=(/dim_k/), all=.true. )
  call mpi_grid_reduce( evalfv_tmp(1,1),     nstfv*nkpt,          dims=(/dim_k/), all=.true. )
  call mpi_grid_reduce( evecfv_tmp(1,1,1),   nmatmax*nstfv*nkpt,  dims=(/dim_k/), all=.true. )
  ! pass the tmp arrays, then free the tmp arrays, only considering non-spinsprl for now
  if (.not.spinsprl) then 
      do ik=1,nkpt
        evalfv_sirius(:,1,ik)   = evalfv_tmp(:,ik)  
        evecfv_sirius(:,:,1,ik) = evecfv_tmp(:,:,ik)
      enddo
  else 
  ...
  ! zero some of the eigen value/vector arrays again before EP SCF run
...
!#endif
  ...
  ! 
  ! NOTE: evalsv for all k-points is already there, can be directly used to compare with evalsv_sirius. 
  ...
  !       copy of the (newly defined) evalfv of all k-points, then use "mpi_grid_reduce" on evalfv. 
  ! 
  !       The same idea for fv eigen vectors, index back from "ikloc" to global "ik" (from the existing 
  !       "evecfvloc" to the (newly defined) "evecfv3" on every rank), then use "mpi_grid_reduce" on evecfv3. 
  ...
  if (mpi_grid_side(dims=(/dim_k/))) then
    do i=0,mpi_grid_dim_size(dim_k)-1
      if (mpi_grid_dim_pos(dim_k).eq.i) then
        do ikloc=1,nkptloc
          ik=mpi_grid_map(nkpt,dim_k,loc=ikloc)
          evalfv(:,:,ik)    = evalfvloc(:,:,ikloc)
          evecfv3(:,:,:,ik) = evecfvloc(:,:,:,ikloc)
        end do
      end if
      call mpi_grid_barrier(dims=(/dim_k/))
    end do
  endif
  call mpi_grid_reduce( evalfv(1,1,1),    nstfv*nspnfv*nkpt,         dims=(/dim_k/), all=.true. )
  call mpi_grid_reduce( evecfv3(1,1,1,1), nmatmax*nstfv*nspnfv*nkpt, dims=(/dim_k/), all=.true. )  
  
  ! debug only
  ! master rank write fv/sv eigen values/vectors of EP and SIRIUS
  if (wproc) then
  
    open(1000, file = 'evalfv_EP_vs_SIRIUS.txt', status = 'new')  
    do ik=1,nkpt 
      write(1000,*) ik
      do is=1,nstfv 
        write(1000,*) is, evalfv(is,1,ik), evalfv_sirius(is,1,ik) 
      enddo ! is
    end do ! ik
    close(1000) 

    open(1001, file = 'evalsv_EP_vs_SIRIUS.txt', status = 'new')  
    do ik=1,nkpt 
      write(1001,*) ik
      do is=1,nstsv 
        write(1001,*) is, evalsv(is,ik), evalsv_sirius(is,ik)
      enddo ! is
    end do ! ik
    close(1001) 
    
    ! only print the fv eigen vectors for ik=1 && nspnfv=1
    open(1002, file = 'evecfv_EP_vs_SIRIUS.txt', status = 'new')   
    do is=1,nstsv 
      write(1002,*) is 
      do inmat=1,nmatmax
        write(1002,*) inmat, evecfv3(inmat,is,1,1), evecfv_sirius(inmat,is,1,1)
      enddo ! is
    end do ! is
    close(1002) 
