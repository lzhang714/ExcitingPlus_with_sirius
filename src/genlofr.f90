...
!BOC
  ...
  real(8) p0s(nrmtmax),q0s(nrmtmax),q1s(nrmtmax)                  ! EXCITING has additional p1s
  real(8) hp0(nrmtmax)
  ...
  ! EXCITING has additional stuff here. 
  ...
  do ia=1,natoms(is)
    if (.not.done(ia)) then
      ...
      ! single index "ias" replaces ia and is
      ias=idxas(ia,is)
      ...
      do ilo=1,nlorb(is)
        ...
        call rschrodapp(solsc,l,nr,spr(:,is),vr,p0s,q0s,q1s,hp0)   ! LZ, this is different from EXCITING

        ! divide by r and store in global array
        do ir=1,nr
          t1=1.d0/spr(ir,is)
          lofr(ir,1,ilo,ias)=t1*p0s(ir)
          lofr(ir,2,ilo,ias)=t1*hp0(ir)    ! EXCITING uses p0s and p1s
        end do
