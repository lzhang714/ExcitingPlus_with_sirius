...
#ifdef _SIRIUS_
    call sirius_get_energy(gs_handler, string("vxc"), engyvxc)
#else
    stop sirius_error
#endif
  else
...
!-----------------------------------------------------!
  ...
  if (use_sirius_library.and.use_sirius_vxc) then
#ifdef _SIRIUS_
    call sirius_get_energy(gs_handler, string("bxc"), engybxc)
#else
    stop sirius_error
#endif
  else
    engybxc = 0.d0
    do idm=1,ndmag
      engybxc = engybxc + rfinp(1, magmt(:,:,:,idm), bxcmt(:,:,:,idm),magir(:,idm), bxcir(:,idm))
    enddo
  endif
...
#ifdef _SIRIUS_
    call sirius_get_energy(gs_handler, string("vha"), engyvcl)
#else
    stop sirius_error
#endif
  else
...
!-----------------------!
  ...
  if (use_sirius_library.and.use_sirius_vha) then
#ifdef _SIRIUS_
    call sirius_get_energy(gs_handler, string("enuc"), engymad)
#else
    stop sirius_error
#endif
  else
    ...
    do is=1,nspecies
      ...
      ! EXCITING: Call potnucl (input%groundstate%ptnucl, 50, spr(:, is), spzn(is), v2)
      call potnucl(ptnucl,1,spr(:,is),spzn(is),vn)
      do ia=1, natoms(is)
        ias=idxas(ia,is)
        ! EXCITING: engymad = engymad + 0.5d0 * spzn (is) * vmad(ias) 
        ! EXCITING: !((vclmt(1,1,ias)*y00-v2(1))*spr(2,is)-(vclmt(1,2,ias)*y00-v2(2))*spr(1,is))/(spr(2,is)-spr(1,is))
...
!-----------------------------------------------!
  ...
  if (use_sirius_library.and.use_sirius_vxc) then
      ...
      !---------------- SIRIUS exchange (+correlation?)  
      engyx = 0.0
      engyc = 0.0
#ifdef _SIRIUS_
      call sirius_get_energy(gs_handler, string("exc"), engyx)
#else
      stop sirius_error
#endif
